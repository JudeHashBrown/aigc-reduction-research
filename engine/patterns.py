...
SENTENCE_RULES = [
    Rule("S01", "顿号罗列过密", "medium", 1.8,
         "一个分句里三项以上顿号并列。lieflat 实测 AI 3.21/千字、人类 1.78，只有 1.8x，"
         "处于「需额外条件才收」的边缘带，所以降为 medium。"
         "本规则额外要求各项等长（对称感），这个条件无人测过，预测力待标定。"
         "注意：句内同构排比（「更高效、更专注」）实测人类比 AI 多 1.6 倍，不在本规则范围。",
...
LIEFLAT_SENTENCE_RULES = [
    Rule("L07", "句首连接词当路标", "medium", 1.5,
         "「然而，」「因此，」「此外，」领起全句。实测 AI 0.18/千字、人类 0.03（6x），人类侧稳定偏低。"
         "改法：把连接词移到主语后面，或换成「不过」「其实」「也」。"
         "不改：论证确实需要在段落转折处立显式路标时。",
